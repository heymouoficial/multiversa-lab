...
describe('Realtime Sync Logic', () => {
    it('should subscribe to tasks, clients, and services tables', () => {
        ...
        // Check subscriptions
        expect(channelMock.on).toHaveBeenCalledWith('postgres_changes', expect.objectContaining({ table: 'tasks' }), expect.any(Function));
        expect(channelMock.on).toHaveBeenCalledWith('postgres_changes', expect.objectContaining({ table: 'clients' }), expect.any(Function));
        expect(channelMock.on).toHaveBeenCalledWith('postgres_changes', expect.objectContaining({ table: 'services' }), expect.any(Function));
